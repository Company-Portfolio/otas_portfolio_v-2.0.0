export const content = {
  nav: {
    logo: "/images/otas-logo.png",
    links: [
      { label: "OTAS အကြောင်း", href: "/about" },
      { label: "ရနိုင်တဲ့ Service များ", href: "/services" },
      // { label: "ပ‌ရောဂျက်များ", href: "/projects" },
      { label: "လုပ်ငန်းများ", href: "/clients" },
      // { label: "ဆက်သွယ်ဆွေးနွေးမယ်", href: "/contact" },
    ],
    cta: "အသေးစိတ် ဆွေးနွေးမယ်",
    language: {
      code: "MM",
      label: "ဘာသာစကား",
    },
  },
  hero: {
    title:
      "လုပ်ငန်းရှင်တွေရဲ့ အခက်အခဲတွေကို \nနည်းပညာနဲ့ ဖြေရှင်းပေးမဲ့ အိုတက်စ်",
    backgroundImage: "/images/HeroBanner.png",
    cta1: "ဆက်သွယ်ဆွေးနွေးမယ်",
    cta2: "ရက်ချိန်းယူမယ်",
  },
  services: {
    badge: "ရနိုင်တဲ့ Service များ",
    description:
      "အိုတက်စ် အနေနဲ့ လုပ်ငန်းရှင်တွေရဲ့ ကြုံတွေ့နေရတဲ့ အခက်အခဲတွေကို သေသေချာချာ ဆွေးနွေးတိုင်ပင် အဖြေရှာပြီး အကောင်းမွန်ဆုံး ရလဒ်များရရှိအောင် ကူညီပေးနေပါတယ်",
    items: [
      {
        id: 1,
        slug: "ai-agent",
        title: "AI Agents for Automation",
        description:
          "လုပ်ငန်းတွေကို အလိုအလျောက် လုပ်ဆောင်ပေးနိုင်တဲ့ AI Agents များ",
        icon: "bot",
      },
      {
        id: 2,
        slug: "customize-software",
        title: "Customize Software for Business Problems",
        description:
          "လုပ်ငန်းရဲ့ အခက်အခဲတွေကို ဖြေရှင်းပေးနိုင်တဲ့ Customize Software များ",
        icon: "code",
      },
      {
        id: 3,
        slug: "business-email",
        title: "Business Email for Brand Identity",
        description:
          "လုပ်ငန်းရဲ့ Brand Identity ကို ပိုခိုင်မာစေတဲ့ Business Email များ",
        icon: "mail",
      },
      {
        id: 4,
        slug: "business-website",
        title: "Business Website for Brand Identity",
        description:
          "လုပ်ငန်းရဲ့ Brand Identity ကို ပိုခိုင်မာစေပြီး Customer တွေရဲ့ ယုံကြည်မှုတိုးစေတဲ့ Business Website များ",
        icon: "globe",
      },
    ],
  },
  caseStudies: {
    title: "အိုတက်စ်မှ ဖန်တီးပေးထားတဲ့ ပရောဂျက်အချို့",
    items: [
      {
        id: 1,
        company: "UEDC Myanmar",
        logo: "/images/logo4.png",
        description:
          "အလိုအလျောက် အော်ဒါကောက်ပြီး အလွယ်တကူ စီမံခန့်ခွဲနိုင်တဲ့ Software",
        image: "/images/caseone.png",
        //         accentColor: "#05C5C5",
      },
      {
        id: 2,
        company: "International Focus Myanmar",
        logo: "/images/logo (3).png",
        description:
          "လုပ်ငန်းလည်ပတ်မှု ပိုမိုလွယ်ကူမြန်ဆန်အောင်ပြုလုပ်ပြီး အချက်အလက်များကို စနစ်တကျ သိမ်းဆည်း ခန့်ခွဲပေးသော Software",
        image: "/images/casethree.png",
        accentColor: "#N2E8DZ",
      },
      {
        id: 3,
        company: "Live Code Myanmar",
        logo: "/images/logo (1).png",
        description:
          "Live Sale မှ အော်ဒါကို အလိုလျောက်ကောက်ခံပေးပြီး Live Saleတစ်ခုလုံးကို စီမံခန့်ခွဲပေးတဲ့ Automation Software",
        image: "/images/casefour.png",
        //         accentColor: "#FF5A5F",
      },
      {
        id: 4,
        company: "OK GROUP OF COMPAINES",
        logo: "/images/logo (2).png",
        description:
          "လုပ်ငန်းရဲ့ Brand Identity ကို ပိုခိုင်မာစေပြီး Customer တွေရဲ့ ယုံကြည်မှုတိုးစေတဲ့ Business Website နှင့် Dash Board",
        image: "/images/casetwo.png",
        //         accentColor: "#034D7E",
      },
    ],
  },
  cta: {
    title: "လုပ်ငန်းတွေရဲ့ အခက်အခဲတွေအတွက် အိုတက်စ် ရဲ့ အဖြေများ",
    cards: [
      {
        id: 1,
        title:
          "လုပ်ငန်းရှင်တွေကို စျေးကူရောင်းပေးမဲ့ အိုတက်စ်ရဲ့ အေအိုင် အရောင်း ဝန်ထမ်း",
        cta1: "Auto Shop စမ်းသုံးကြည့်မယ်",
        cta1Link: "https://autoshopmm.com",
        cta2: "ဆက်သွယ်ဆွေးနွေးမယ်",
        cta2Link: "https://www.facebook.com/autoshopmm",
        image: "/images/Group-1.png",
        variant: "primary",
      },
      {
        id: 2,
        title: "ဝန်ထမ်းနည်းနည်း ဘတ်ဂျတ်နည်းနည်းနဲ့ ဖွင့်နိုင်တဲ့ Online ဆိုင်ခွဲ Auto Shop",
        cta1: "Auto Shop စမ်းသုံးကြည့်မယ်",
        cta1Link: "https://autoshopmm.com",
        cta2: "ဆက်သွယ်ဆွေးနွေးမယ်",
        cta2Link: "https://www.facebook.com/autoshopmm",
        image: "/images/image-6.png",
        variant: "secondary",
      },
    ],
  },
  clientsLogo: {
    title: "အိုတက်စ် နှင့် လက်တွဲထားသော လုပ်ငန်းများ",
    viewAll: "လုပ်ငန်း အားလုံးကို ကြည့်မယ်",
    items: [
      { id: 1, name: "UEDC Myanmar", logo: "/images/clientlogo/clientLogo (1).svg" },
      {
        id: 2,
        name: "Suzuki Mega 6 Car showroom",
        logo: "/images/clientlogo/clientLogo (2).svg",
      },
      { id: 3, name: "Live Code Myanmar", logo: "/images/live-code-logo.svg" },
      {
        id: 4,
        name: "International Focus",
        logo: "/images/clientlogo/clientLogo (3).svg",
      },
      { id: 5, name: "Funglish Max", logo: "/images/clientlogo/clientLogo (4).svg" },
      {
        id: 6,
        name: "OK Group of Compaines",
        logo: "/images/clientlogo/clientLogo (5).svg",
      },
      {
        id: 7,
        name: "Digital Saw Marketing Agency",
        logo: "/images/digital-saw.png",
      },
      {
        id: 8,
        name: "Medical Education Guild",
        logo: "/images/client-medical.png",
      },
      { id: 9, name: "Drinking Mode", logo: "/images/clientlogo/clientLogo (21).svg" },
      {
        id: 10,
        name: "General Impex",
        logo: "/images/clientlogo/clientLogo (20).svg",
      },
    ],
  },
  faq: {
    title: "မေးခွန်းနဲ့ အဖြေများ",
    subtitle: "အိုတက်ခ်ကို လုပ်ငန်းရှင်တွေအမေးများသော မေးခွန်းများ",
    tabs: [
      { id: "website", label: "Business Website ဆိုင်ရာ" },
      { id: "software", label: "Customize Software ဆိုင်ရာ" },
      { id: "email", label: "Business Email ဆိုင်ရာ" },
      { id: "ai", label: "AI Agent ဆိုင်ရာ" },
    ],
    items: {
      website: [
        {
          question: "Business Website ဆိုတာ ဘာလဲ?",
          answer:
            "လုပ်ငန်းရဲ့ Brand Identity ကို ပိုခိုင်မာစေပြီး Customer တွေရဲ့ ယုံကြည်မှုတိုးစေတဲ့ Website ဖြစ်ပါတယ်။",
        },
        {
          question: "Business Website ရဲ့ အကျိုးကျေးဇူးတွေ ဘာတွေလဲ?",
          answer:
            "လုပ်ငန်းရဲ့ Online Presence တိုးလာပြီး Customer တွေရဲ့ ယုံကြည်မှု ပိုရလာပါတယ်။",
        },
        {
          question: "ဘယ်လောက်ကြာအောင် လုပ်ပေးလဲ?",
          answer: "ပုံမှန်အားဖြင့် ၂ ပတ်မှ ၄ ပတ်အထိ ကြာနိုင်ပါတယ်။",
        },
        {
          question: "ဘယ်လောက် ကုန်ကျနိုင်လဲ?",
          answer:
            "လုပ်ငန်းရဲ့ လိုအပ်ချက်ပေါ်မူတည်ပြီး ကွာခြားနိုင်ပါတယ်။ ကျွန်တော်တို့ကို ဆက်သွယ်ပြီး အသေးစိတ် ဆွေးနွေးနိုင်ပါတယ်။",
        },
      ],
      software: [
        {
          question: "Customize Software ဆိုတာ ဘာလဲ?",
          answer:
            "လုပ်ငန်းရဲ့ အခက်အခဲတွေကို ဖြေရှင်းပေးနိုင်တဲ့ Software ဖြစ်ပါတယ်။",
        },
        {
          question: "ဘယ်လို Software တွေ လုပ်ပေးနိုင်လဲ?",
          answer:
            "လုပ်ငန်းရဲ့ လိုအပ်ချက်ပေါ်မူတည်ပြီး ERP, CRM, Inventory Management စတာတွေ လုပ်ပေးနိုင်ပါတယ်။",
        },
      ],
      email: [
        {
          question: "Business Email ဆိုတာ ဘာလဲ?",
          answer:
            "လုပ်ငန်းရဲ့ Brand Identity ကို ပိုခိုင်မာစေတဲ့ Email ဖြစ်ပါတယ်။",
        },
        {
          question: "Business Email ရဲ့ အကျိုးကျေးဇူးတွေ ဘာတွေလဲ?",
          answer:
            "လုပ်ငန်းရဲ့ ယုံကြည်မှု ပိုတိုးလာပြီး Professional ဖြစ်ပါတယ်။",
        },
      ],
      ai: [
        {
          question: "AI Agent ဆိုတာ ဘာလဲ?",
          answer:
            "လုပ်ငန်းတွေကို အလိုအလျောက် လုပ်ဆောင်ပေးနိုင်တဲ့ AI ဖြစ်ပါတယ်။",
        },
        {
          question: "AI Agent က ဘာတွေ လုပ်ပေးနိုင်လဲ?",
          answer:
            "Customer Service, Sales Automation, Data Analysis စတာတွေ လုပ်ပေးနိုင်ပါတယ်။",
        },
      ],
    },
  },
  success: {
    badge: "အောင်မြင်မှု မှတ်တမ်းများ",
    subTitle:
      "အိုတက်စ် အနေနဲ့ လုပ်ငန်းရှင်တွေရဲ့ အခက်အခဲတွေကို ဆွေးနွေးတိုင်ပင်အဖြေရှာပြီး အကောင်းဆုံး ရလဒ်များရအောင် ဖန်တီးပေးနိုင်ခဲ့ပါတယ်",
    cards: [
      {
        id: 1,
        title: "UEDC Myanmar",
        description:
          "လုပ်ငန်းသုံးအ‌ရောင်း စီမံခန့်ခွဲ‌ရေး software ဖြင့် အော်ဒါကောက် ကြန့်ကြာနေမှုပြသာနာကို ဖြေရှင်းပေးနိုင်ခဲ့ပါတယ်",
        cta1: "ဘယ်လိုပြောင်းလဲသွားလဲ ကြည့်မယ်",
        image: "/images/MEGA.png",
        variant: "primary",
      },
      {
        id: 2,
        title: "Mi Mi Cosmetic",
        description:
          "အော်ဒါ စီမံခန့်ခွဲ‌ရေး သီးသန့် software ဖြင့် အရောင်းအဝယ် စာရင်းကို စနစ်တကျ မှတ်တမ်းတင်ပေးပြီး စာရင်းတွက်ရတဲ့ အခက်အခဲကို ဖြေရှင်း‌ပေးနိုင်ခဲ့ပါတယ်",
        cta1: "ဘယ်လိုပြောင်းလဲသွားလဲ ကြည့်မယ်",
        image: "/images/mimi.png",
        variant: "secondary",
      },
      {
        id: 3,
        title: "Healthy Nara",
        description:
          "လုပ်ငန်းသုံးစီမံခန့်ခွဲ‌ရေး software ဖြင့် Healthy Nara ရဲ့ လုပ်ငန်းဆိုင်ရာ အချိန်ကြန့်ကြာမှုများကို လျော့ချကာ မြန်ဆန်စွာ လုပ်ဆောင်နိုင်ရန် ကူညီပေးခဲ့ပါတယ်",
        cta1: "ဘယ်လိုပြောင်းလဲသွားလဲ ကြည့်မယ်",
        image: "/images/healthyNara.png",
        variant: "secondary",
      },
    ],
  },

  about: {
    hero: {
      title:
        "လူကြီးမင်းတို့ရဲ့ လုပ်ငန်းတွေကို နည်းပညာနဲ့ \nတိုးတက်အောင် ကူညီပေးမဲ့ အိုတက်စ်",
      image: "/images/AboutBannerSection.png",
    },
    journey: {
      title: "အိုတက်စ်ရဲ့ ခရီးစဉ်",
      image: "/images/aboutcompany.png",
      stats: [
        { label: "အိုတက်စ်ရဲ့ ခရီးစဉ်", value: "5 နှစ်ကြာ" },
        { label: "ကူညီဆွေးနွေးပေးထားသော", value: "လုပ်ငန်းရှင် 200+" },
      ],
    },
    purpose: {
      badge: "အိုတက်စ်ရဲ့ရည်ရွယ်ချက်",
      description:
        "အိုတက်စ်ရဲ့ အဓိက ရည်ရွယ်ချက်က လုပ်ငန်းရှင်တွေ ကြုံတွေ့နေရတဲ့ အခက်အခဲတွေကို အကုန်အကျအနည်းဆုံးနဲ့ အချိန်တိုအတွင်း ထိရောက်စွာဖြေရှင်းပေးနိုင်ဖို့ ဖြစ်ပါတယ်",
      image: "/images/image63.png",
    },
    process: {
      title: "ဘာလို့ရွေးချယ်သင့်တာလဲ",
      description:
        "အိုတက်စ်ရဲ့ အဓိက ရည်ရွယ်ချက်က လုပ်ငန်းရှင်တွေ ကြုံတွေနေရတဲ့ အခက်အခဲတွေကို နည်းပညာအသုံးပြုပြီး အထိရောက်ဆုံး ဖြေရှင်းပေးနိုင်ဖို့ ဖြစ်ပါတယ်",
      steps: [
        {
          step: "ပထမအဆင့်",
          title: "ဆွေးနွေးတိုင်ပင်မယ်",
          description:
            "အိုတက်စ် အနေနဲ့ သင့်လုပ်ငန်းရဲ့ အခက်အခဲကို တိကျစွာသိရှိပြီး အထိရောက်ဆုံး ကူညီပေးနိုင်ဖို့အတွက် လုပ်ငန်းနဲ့ပတ်သက်ပြီး ဆွေးနွေးတိုင်ပင်ပေးပါတယ်",
        },
        {
          step: "ဒုတိယအဆင့်",
          title: "အဖြေရှာမယ်",
          description:
            "အိုတက်စ် အနေနဲ့ သင့်လုပ်ငန်းရဲ့ အခက်အခဲကို သိပြီးတာနဲ့ ဖြေရှင်းဖို့ အထိရောက်ဆုံးနဲ့ အသက်သာဆုံး နည်းလမ်းတွေကို ရှာဖွေပြီး တိုင်ပင်ဆွေးနွေးပေးပါတယ်",
        },
        {
          step: "တတိယအဆင့်",
          title: "ပြဿနာကို ဖြေရှင်းမယ်",
          description:
            "အထိရောက်ဆုံး နည်းလမ်းကို ရှာဖွေပြီးတာနဲ့ အကောင်အထည်ပေါ်လာအောင် Design ကနေပြီး Development အဆင့်ထိ တစ်ဆင့်ပြီးတစ်ဆင့် တည်ဆောက်ပေးပါတယ်",
        },
        {
          step: "စတုတ္ထအဆင့်",
          title: "ထိရောက်မှုရှိ မရှိ စမ်းသပ်မယ်",
          description:
            "တည်ဆောက်ထားတဲ့ အဖြေက လုပ်ငန်းအတွက်ကိုက်ညီမှု ရှိမရှိသေချာစေဖို့ လက်တွေ့စမ်းသပ်ပေးပါတယ်",
        },
        {
          step: "နောက်ဆုံးအဆင့်",
          title: "လုပ်ငန်းရှင်လက်ထဲ လက်လွှဲပေးမယ်",
          description:
            "အိုတက်စ်အနေနဲ့ စိတ်ချရတဲ့ စမ်းသပ်မှုတွေ လုပ်ပြီးတဲ့နောက် အသုံးပြုရတဲ့ နည်းလမ်းတွေ သတိထားရတဲ့ အချက်တွေကို လက်တွေ့ကူညီ ပြသပေးပြီး လုပ်ငန်းရှင်လက်ထဲ လွှဲပြောင်းပေးပါတယ်",
        },
        {
          step: "အပိုအဆင့်",
          title: "အချိန်တိုင်း အသင့်ရှိနေပါမယ်",
          description:
            "လုပ်ငန်းရှင် လက်ထဲကို လွှဲပြောင်းပေးပြီးတဲ့နောက်မှာလည်း အိုတက်စ်အနေနဲ့ လုပ်ငန်းရှင်တွေရဲ့ နည်းပညာ အခက်အခဲတွေကို ကူညီဖြေရှင်းပေးဖို့ အမြဲအသင့်ရှိနေပါတယ်",
        },
      ],
    },

    helpCustomers: {
      title: "အိုတက်စ်မှ ကူညီဖြေရှင်းပေးထားတဲ့ လုပ်ငန်းရှင်များ",
      image: [
        "/images/image64.png",
        "/images/image65.png",
        "/images/image66.png",
      ],
    },

    clientHelp: {
      title: "အိုတက်စ်မှ ကူညီဖြေရှင်းပေးထားတဲ့ လုပ်ငန်းရှင်များ",
      items: [
        { id: 1, name: "UEDC MYANMAR", logo: "/images/clientlogo/clientLogo (1).svg" },
        { id: 2, name: "SUZUKI MEGA 6 CAR SHOWROOM", logo: "/images/clientlogo/clientLogo (2).svg" },
        { id: 3, name: "INTERNATIONAL FOCUS", logo: "/images/clientlogo/clientLogo (3).svg" },
        { id: 4, name: "FUNGLISH MAX", logo: "/images/clientlogo/clientLogo (4).svg" },
        { id: 5, name: "OK GROUP OF COMPAINES", logo: "/images/clientlogo/clientLogo (5).svg" },
        { id: 6, name: "DRINKING MODE", logo: "/images/clientlogo/clientLogo (21).svg" },
        { id: 7, name: "GENERAL IMPEX", logo: "/images/clientlogo/clientLogo (20).svg" },
        { id: 8, name: "GOLDEN MYANMAR HUMAN RESOURCES", logo: "/images/clientlogo/clientLogo (19).svg" },
        { id: 9, name: "ME ME COSMETICS", logo: "/images/clientlogo/clientLogo (18).svg" },
        { id: 10, name: "TOP NOTCH", logo: "/images/clientlogo/clientLogo (17).svg" },
        { id: 11, name: "IMAS", logo: "/images/clientlogo/clientLogo (16).svg" },
        { id: 12, name: "KAUNG KADAY KYWAL CONSTRUCTION", logo: "/images/clientlogo/clientLogo (15).svg" },
        { id: 13, name: "GOLD MINE INTERNATIONAL CO.,LTD", logo: "/images/clientlogo/clientLogo (14).svg" },
        { id: 14, name: "SHWE PYI", logo: "/images/clientlogo/clientLogo (13).svg" },
        { id: 15, name: "KO MIN DIY STORE", logo: "/images/clientlogo/clientLogo (12).svg" },
        { id: 16, name: "HONGCHI", logo: "/images/clientlogo/clientLogo (11).svg" },
        { id: 17, name: "CHANGAN MYANMAR", logo: "/images/clientlogo/clientLogo (10).svg" },
        { id: 18, name: "AVENUE 101", logo: "/images/clientlogo/clientLogo (9).svg" },
        { id: 19, name: "LIQUOR PRO", logo: "/images/clientlogo/clientLogo (8).svg" },
        { id: 20, name: "168 SUCCESS REAL ESTATE AGENCY", logo: "/images/clientlogo/clientLogo (7).svg" }
      ],
    },
    partners: {
      title: "အိုတက်စ်နဲ့ ပါတနာအဖြစ် လက်တွဲထားသော လုပ်ငန်းများ",
      items: [
        { id: 1, name: "DIGITAL SAW MARKETING AGENCY", logo: "/images/partaner/partanar (4).png" },
        { id: 2, name: "MEDICAL EDUCATION GUILD", logo: "/images/partaner/partanar (3).png" },
        { id: 3, name: "LIVE CODE MYANMAR", logo: "/images/partaner/partanar (2).png" },
        { id: 4, name: "HEALTHY NARA", logo: "/images/partaner/partanar (1).png" }
      ]
    },
    testimonials: {
      title: "အိုတက်စ်အပေါ် လုပ်ငန်းရှင်တွေရဲ့ စကားသံများ",
      items: [
        {
          id: 1,
          name: "ကိုဖိုက်",
          position: "UEDC MYANMAR FOUNDER",
          quote:
            "အိုတက်စ်ဆီက Customize Software ယူပြီးတဲ့နောက် ပစ္စည်းတွေ အော်ဒါကောက်တာ ပို့ပေးဖို့ကြန့်ကြာနေတဲ့ အပိုင်းတွေအများကြီးသက်သာသွားတယ်ဗျ ",
          image: "/images/image67.png",
          logo: "/images/logo-uedc.png",
        },
        {
          id: 2,
          name: "မလင်းလင်း",
          position: "GENERAL MANAGER / CHANGAN MYANMAR",
          quote: "အိုတက်စ်နဲ့ အလုပ်တွဲလုပ်ရတာ အများကြီး အဆင်ပြေပါတယ်ဗျ",
          image: "/images/image68.png",
          logo: "/images/client-suzuki.svg",
        },
        {
          id: 3,
          name: "ကိုကျော်မင်း",
          position: "CEO / SUZUKI MEGA 6 MYANMAR",
          quote:
            "အိုတက်စ်နဲ့ အလုပ်လုပ်ရတာ စိတ်ချရပြီး စိတ်ကျေနပ်မှု အပြည့်အဝရရှိပါတယ်",
          image: "/images/casetwo.png",
          logo: "/images/client-suzuki.svg",
        },
      ],
    },
  },

  service: {
    banner: "/images/ServiceBannerSection.png",
    details: [
      {
        id: 1,
        slug: "business-website",
        title: "Business Website",
        subtitle: "လုပ်ငန်းကို ယုံကြည်မှုတိုးစေမဲ့",
        description:
          "လုပ်ငန်းရှင်တွေရဲ့ လုပ်ငန်းအတွက် Branding နှင့် Customer များ ၊ အလုပ်လက်တွဲလုပ်ဆောင်မည့် စီးပွားရေး လုပ်ငန်းများ၏ ယုံကြည်မှု ခိုင်မာစေဖို့အတွက် လုပ်ငန်းများအတွက် Business Portfolio ရရှိမှာ ဖြစ်ပါတယ်",
        image: "/images/image.png",
        benefits: [
          "- ကိုယ်ပိုင် Domain နှင့် Hosting ဖြင့် လုပ်ငန်း၏ ယုံကြည်မှုကို တိုးတက်စေခြင်း",
          "- သန့်ရှင်းသပ်ရပ်ပြီး Professional ကျသော Design များဖြင့် ဖန်တီးပေးခြင်း",
          "- Google SEO အပြည့်အဝ ထည့်သွင်းပေးထားသဖြင့် ရှာဖွေရလွယ်ကူခြင်း",
          "- Responsive Design ဖြစ်သဖြင့် မည်သည့် Device တွင်မဆို အဆင်ပြေစွာကြည့်ရှုနိုင်ခြင်း",
        ],
      },
      {
        id: 2,
        slug: "business-email",
        title: "Business Email",
        subtitle: "လုပ်ငန်းရဲ့ ပုံရိပ်ကို မြှင့်တင်ပေးမဲ့",
        description:
          "လုပ်ငန်းရှင်တွေရဲ့ လုပ်ငန်းအတွက် B2B, B2G လုပ်ငန်းအမျိုးအစားများ ဆက်သွယ်ဆောင်ရွက်ရန်အတွက် အထူးသင့်လျော်ပြီး၊ လုပ်ငန်း၏ လုံခြုံစိတ်ချရမှုနှင့် ပရော်ဖက်ရှင်နယ်ကျမှုကို အပြည့်အဝ မြှင့်တင်ပေးမည့် Business Email ဖြစ်ပါတယ်",
        image: "/images/image2.png",
        benefits: [
          "- ကိုယ်ပိုင်လုပ်ငန်းအမည်ဖြင့် အသုံးပြုနိုင်၍ B2B, B2G နှင့် Client များ၏ ယုံကြည်မှုကို ခိုင်မာစေခြင်း။",
          "- ဝန်ထမ်းများ၏ အကောင့်များကို အပြည့်အဝ စီမံနိုင်ပြီး Data ဆုံးရှုံးမှု၊ မသမာမှုများကို ကာကွယ်ပေးနိုင်ခြင်း။",
          "- Google SMS OTP ပြဿနာကို ကျော်လွှားနိုင်ပြီး အကောင့်ပြန်ရလွယ်ကူခြင်း။",
          "- Built-in AI Assistant ပါဝင်သဖြင့် ရုံးသုံး Email များကို အချိန်တိုအတွင်း လွယ်ကူစွာ ရေးသားနိုင်ခြင်း။",
        ],
      },
      {
        id: 3,
        slug: "ai-agent",
        title: "AI Agent",
        subtitle: "လုပ်ငန်းရဲ့ အချိန်ကို ၈ဆ မြှင့်တင်ပေးမဲ့",
        description:
          "လုပ်ငန်းရှင်တွေရဲ့ လုပ်ငန်းအတွက် Social Media ကနေ အရောင်းပိုင်း ၊ ပစ္စည်းအကြောင်းရှင်းပြခြင်းနှင့် Customer Service လုပ်ငန်းများကို အကောင်းဆုံးနဲ့ အမြန်ဆုံး (24/7) ဝန်ဆောင်မှုပေးနိုင်တဲ့ AI အရောင်းဝန်ထမ်း ရရှိမှာ ဖြစ်ပါတယ်",
        image: "/images/image3.png",
        benefits: [
          "- လုပ်ငန်းလည်ပတ်မှုနှုန်းကို ၈ဆအထိ မြှင့်တင်ပေးပြီး အကောင်းဆုံး Customer Experience ကို ပေးစွမ်းနိုင်ခြင်း",
          "- Facebook, TikTok နှင့် ကိုယ်ပိုင် App များတွင် တိုက်ရိုက်အသုံးပြုနိုင်သော AI Agent တစ်ယောက် ရရှိခြင်း",
          "- Customer များအတွက် အမြန်ဆန်ဆုံးနှင့် စိတ်ကျေနပ်မှုအပြည့်အဝ ဝန်ဆောင်မှုကို အပြည့်အဝ ပေးနိုင်ခြင်း",
          "- ပိတ်ရက်မရှိ (24/7) အော်ဒါကောက်ခြင်း၊ မေးခွန်းဖြေခြင်းနှင့် အခက်အခဲများကို ချက်ချင်းဖြေရှင်းပေးနိုင်ခြင်း",
        ],
      },
      {
        id: 4,
        slug: "customize-software",
        title: "Customize Software",
        subtitle: "သင့်လုပ်ငန်းရဲ့ အခက်အခဲကို တစ်ချက်တည်းနဲ့ ဖြေရှင်းပေးမဲ့",
        description:
          "လုပ်ငန်းရှင်တွေရဲ့ ကြုံတွေ့နေရတဲ့ လုပ်ငန်းအခက်အခဲတွေကို အမှန်တကယ် ဖြေရှင်းပေးနိုင်သည့် Software သာ ဖြစ်သည်ဟူသော ယုံကြည်ချက်ဖြင့် လုပ်ငန်းသုံး Software များကို အစမှအဆုံး ရေးဆွဲဖန်တီးပေးသော ဝန်ဆောင်မှုဖြစ်ပါသည်။",
        image: "/images/image4.png",
        benefits: [
          "- လုပ်ငန်းသုံး Software များကို မိမိလုပ်ငန်းလိုအပ်ချက်နှင့် ကိုက်ညီအောင် စိတ်ကြိုက် ရေးဆွဲနိုင်ခြင်း။",
          "- ကြန့်ကြာနေသော Manual လုပ်ငန်းစဉ်များကို အလိုအလျောက်စနစ်ဖြင့် အစားထိုးကာ လုပ်ငန်းအချိန်နှင့်အရှိန်ကို မြှင့်တင်ပေးနိုင်ခြင်း။",
          "- စနစ်ကျွမ်းကျင်စွာ အသုံးပြုနိုင်သည့်အထိ အနီးကပ် ပံ့ပိုးကူညီပေးမည့် သီးသန့် Technical Support Team ၏ ဝန်ဆောင်မှုကို ရရှိခြင်း။",
          "- ပြောင်းလဲလာသော Сီးပွားရေး လိုအပ်ချက်များနှင့်အညီ Software ကို ရေရှည်အတွက် စဉ်ဆက်မပြတ် Updates ပြုလုပ်ပေးခြင်း။",
        ],
      },
    ],
  },


  clients: {
    banner: "/images/ClientBannerSection.png",
    title: "အိုတက်စ်မှတာဝန်ယူ ဆောင်ရွက်ပေးခဲ့တဲ့ ပရောဂျက်အချို့",
    items: [
      {
        id: 1,
        name: "UEDC MYANMAR",
        description: "အော်ဒါ အလိုအလျောက်ကောက်ပြီး စာရင်းသွင်း စီမံပေးတဲ့ လုပ်ငန်းသုံး အရောင်း Sale Bot",
        logo: "/images/clientlogo/clientLogo (1).svg",
      },
      {
        id: 2,
        name: "SUZUKI MEGA 6 CAR SHOWROOM",
        description: "လုပ်ငန်းရဲ့ ယုံကြည်မှုကို မြှင့်တင်ပေးတဲ့ Business Portfolio",
        logo: "/images/clientlogo/clientLogo (2).svg",
      },
      {
        id: 3,
        name: "LIVE CODE MYANMAR",
        description: "Live Sale ကနေအော်ဒါ အလိုအလျောက်ကောက်ကာ စီမံပေးတဲ့ လုပ်ငန်းသုံး စီမံခန့်ခွဲရေး Software",
        logo: "/images/live-code-logo.svg",
      },
      {
        id: 4,
        name: "INTERNATIONALFOCUS",
        description: "လုပ်ငန်းတာဝန်များကို စနစ်တကျ စီမံခန့်ခွဲပေးပြီး အချက်အလက်များ သိမ်းဆည်းပေးသော စီမံခန့်ခွဲရေး Software",
        logo: "/images/clientlogo/clientLogo (3).svg",
      },
      {
        id: 5,
        name: "OK GROUP OF COMPANIES",
        description: "လုပ်ငန်းရဲ့ ယုံကြည်မှုကို မြှင့်တင်ပေးတဲ့ Business Portfolio",
        logo: "/images/clientlogo/clientLogo (5).svg",
      },
      {
        id: 6,
        name: "MEDICAL EDUCATION GUILD",
        description: "လုပ်ငန်းရဲ့ ယုံကြည်မှုကို မြှင့်တင်ပေးတဲ့ Business Portfolio",
        logo: "/images/client-medical.png",
      },
      {
        id: 7,
        name: "FUNGLISH MAX",
        description: "လုပ်ငန်းရဲ့ ယုံကြည်မှုကို မြှင့်တင်ပေးတဲ့ Business Portfolio",
        logo: "/images/clientlogo/clientLogo (4).svg",
      },
      {
        id: 8,
        name: "DIGITAL SAW MARKETING AGENCY",
        description: "လုပ်ငန်းရဲ့ ယုံကြည်မှုကို မြှင့်တင်ပေးတဲ့ Business Portfolio",
        logo: "/images/digital-saw.png",
      },
      {
        id: 9,
        name: "DRINKING MODE",
        description: "အရက်မူးပြီး မဖြစ်သင့်တဲ့ အရာတွေကို ကြိုတင်ဟန့်တားပေးတဲ့ Customize Software",
        logo: "/images/clientlogo/clientLogo (21).svg",
      },
    ]
  },

  contact: {
    banner: "/images/ContactBannerSection.png"
  },

  footer: {
    cta: "အခုပဲ ဆက်သွယ်စုံစမ်းလိုက်ပါ",
    contact: {
      email: "info@otastechsolutions.com",
      phone: "09 95 120 7795",
    },
    address:
      "No(503), Building (1), 7 Mile Condo, Parami Road, Mayangone Township, Yangon",

    linkHeader: [
      { title1: "အခြားသော link များ" },
      { title2: "ရုံးခန်းလိပ်စာ" },
    ],
    quickLinks: [
      { label: "OTAS အကြောင်း", href: "/about" },
      { label: "ရနိုင်တဲ့ Service များ", href: "/services" },
      // { label: "ပ‌ရောဂျက်များ", href: "/projects" },
      { label: "လုပ်ငန်းရှင်များ", href: "/clients" },
    ],
    social: [
      { name: "Facebook", href: "https://www.facebook.com/otastechsolutions" },
      { name: "LinkedIn", href: "https://www.linkedin.com/company/otas-tech-solution/" },
      { name: "Viber", href: "viber://add?number=+959951207795" },
    ],
    copyright: "© 2026 otastechsolutions.com, All right Reserved",
  },

  homeTestimonials: {
    title: "အိုတက်စ်အပေါ် လုပ်ငန်းရှင်တွေရဲ့ စကားသံများ",
    items: [
      {
        id: 1,
        name: "ကိုကျော်မင်းထွန်း",
        position: "MANAGING DIRECTOR / SUZUKI MEGA 6 MYANMAR",
        quote: "ဆက်ဆံရေးကတော့ တကယ်ကောင်းပါတယ် မရှင်းတာလေးတွေ မေးလိုက်ရင်လည်း စိတ်ရှည်ရှည်နဲ့ သေသေချာချာရှင်းပြ ပေးတယ် လိုအပ်တာတွေ ရှိရင်လည်း အမြဲ ကူညီပေးတာ အရမ်းအဆင်ပြေတယ်။ ဒီတိုင်းလေး ကောင်းမွန်သော service နဲ့ဆို အများကြီး အောင်မြင်လာမှာ ပါ။ နောက်လည်း ဒီထက်မက တိုးတက်အောင်မြင်ပါစေလို့ ဆုမွန်ကောင်းတောင်းပေးပါတယ်။",
        logo: "/images/MEGA.png"
      },
      {
        id: 2,
        name: "မဇင်မီမိထက်",
        position: "(FOUNDER)/ ME ME COSMETICS",
        quote: "OTAS Team က အမြဲစိတ်ရှည်ပြီး အဆင်ပြေ အောင် လုပ်ပေးပါတယ် ရှင့် အခုလုပ်ထားတဲ့ Customize Software လေး က အသုံးပြုရတာ အရမ်း အဆင်ပြေတယ်။ အလုပ်လုပ်ရတာ အရင်ကထက် ပိုမြန်သွားတယ်... Good Service",
        logo: "/images/meme.png"
      },
      {
        id: 3,
        name: "ကိုမင်းလတ်",
        position: "GENERAL MANAGER / OK GROUP OF COMPANIES",
        quote: "စိတ်ရှည်ပေးတဲ့ အချက်ကတော့ တကယ်ကောင်းပါတယ် ခဏခဏ ပြင်ဆင်တာကို သေချာဆောင်ရွက်ပေးတယ် လိုအပ်တာတွေရှိရင်လည်း အမြဲကူညီပေးတာ အရမ်းအဆင်ပြေတယ်။ ဝန်ဆောင်မှုကို ကြယ်ငါးပွင့်အဆင့် ပေးချင်ပါတယ်။",
        logo: "/images/ok.png"
      },
      {
        id: 4,
        name: "မခင်နွယ်ဦး",
        position: "ADMIN EXECUTIVE / GOLDEN MYANMAR HR COMPANY",
        quote: "OTAS Tech Solutions နှင့် အလုပ်လုပ်ရသည်မှာ Professional ကျပြီး အဖွဲ့သားများမှာ စိတ်ရှည် သလို Client လိုအပ်ချက် နှင့်ကိုက်ညီအောင် လုပ်ဆောင်ပေးနိုင်ပါသည်။",
        logo: "/images/gl.png"
      }
    ]
  },

  heroImg: {
    image: "/images/heroImg.png",
    aboutTitle: "လူကြီးမင်းတို့ရဲ့  လုပ်ငန်းတွေကို နည်းပညာနဲ့ တိုးတက်အောင် ကူညီပေးမဲ့ အိုတက်စ်",
    serviceTitle: "အိုတက်စ်က ကူညီဆောင်ရွက်ပေးနေတဲ့ နည်းပညာဆိုင်ရာ Services များ",
    businessPartner: `လုပ်ငန်းရှင်တွေရဲ့အောင်မြင်မှုက အိုတက်စ်ရဲ့ အောင်မြင်မှုနဲ့ ထပ်တူကျပါတယ်`,
    contactTitle: "လုပ်ငန်းရှင်တွေကို အကူအညီပေးဖို့အတွက် အိုတက်စ် က အမြဲအသင့်ရှိနေပါတယ်"
  }


};
